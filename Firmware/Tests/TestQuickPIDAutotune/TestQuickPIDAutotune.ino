...
const uint32_t sampleTimeUs = 400000; // 10ms
...
float outputStep = 5;
float hysteresis = 1;
float setpoint = 150;     // 1/3 of range for symetrical waveform
  ...
  if (_myPID.autoTune) // Avoid dereferencing nullptr after _myPID.clearAutoTune()
  {
    switch (_myPID.autoTune->autoTuneLoop()) {
      case _myPID.autoTune->AUTOTUNE:
        Input = avg((int)thermocouple.readCelsius()); //_myPID.analogReadFast(inputPin)
